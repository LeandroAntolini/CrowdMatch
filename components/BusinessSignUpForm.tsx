...
const BusinessSignUpForm: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.auth.signUp({
            ...
            options: {
                data: {
                    role: 'owner' // Define o papel como 'owner' para contas de lojista
                }
            }
        });
    ...
    if (success) {
        return (
            <div className="text-center p-4 bg-surface rounded-lg">
                <h3 className="font-bold text-lg text-green-400">Cadastro de Lojista realizado!</h3>
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">E-mail do Estabelecimento</label>
                <input
                    type="email"
                    value={email}
