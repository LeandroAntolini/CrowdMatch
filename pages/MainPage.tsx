...
const MainPage: React.FC = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (crowdLevelRef.current && !crowdLevelRef.current.contains(event.target as Node)) {
                setIsCrowdLevelOpen(false);
            }
        };
    ...
    return (
        <div className="p-4">
            ...
            <div className="flex gap-4 mb-6">
                {/* Category Filter */}
                <div className="relative w-1/2" ref={categoryRef}>
                    <button onClick={() => setIsCategoryOpen(!isCategoryOpen)} className="w-full flex items-center justify-between bg-surface px-4 py-2 rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-accent">
                        <span className="truncate">Cat: {selectedCategory}</span>
                ...
                {/* Crowd Level Filter */}
                <div className="relative w-1/2" ref={crowdLevelRef}>
                    <button onClick={() => setIsCrowdLevelOpen(!isCrowdLevelOpen)} className="w-full flex items-center justify-between bg-surface px-4 py-2 rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-accent">
                        <span className="truncate">Lotação: {selectedCrowdLevel}</span>
                        <ChevronDown size={20} className={`transition-transform duration-200 ${isCrowdLevelOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {isCrowdLevelOpen && (
                        <ul className="absolute z-10 w-full mt-1 bg-gray-800 border border-gray-700 rounded-md shadow-lg max-h-60 overflow-y-auto no-scrollbar">
