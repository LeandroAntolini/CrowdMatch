...
const MenuPage: React.FC = () => {
  ...
  /**
   * CORREÇÃO PRINCIPAL:
   * - Se existe tableNumber, a "comanda" deve carregar por (place_id + table_number),
   *   e não por user_id.
   * - Se NÃO existe tableNumber, cai no comportamento antigo (por user_id).
   */
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      if (tableNumber) {
        ...
        // Sem table e sem usuário: não há o que buscar.
        setUserOrders([]);
        return;
      }
    ...
    const loadData = async () => {
      try {
        ...
        if (isAuthenticated && currentUser) {
          // Buscar comanda/pedidos já existentes
          await fetchOrders();
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!placeId || !tableNumber || !currentUser?.id) {
      toast.error('Erro na identificação do local/mesa/usuário.');
      return;
    }
    ...
    try {
      ...
      const { data: orderData, error: orderError } = await supabase
        ...
        .select('id, place_id, user_id, table_number, total_price, status, created_at')
        .single();

      if (orderError) throw orderError;
      if (!orderData?.id) throw new Error('Pedido criado sem ID retornado.');

      // 2) Insere itens do pedido
      const itemsToInsert = cartEntries.map(([itemId, qty]) => {
        const unitPrice = menuItems.find((i) => i.id === itemId)?.price || 0;
        return {
      ...
      // 3) Recarrega comanda da mesa (agora por place_id + table_number)
      await fetchOrders();
    } catch (error: any) {
  ...
  const renderedMenu = useMemo(() => {
    ...
    return categories.map((cat) => (
      <section key={cat} className="animate-fade-in-up mt-6 first:mt-0">
        <h2 className="text-sm font-black text-primary mb-4 border-l-4 border-primary pl-3 uppercase tracking-tighter">
          {cat}
        </h2>
            ...
            .map((item) => (
              <div key={item.id} className="bg-surface rounded-2xl p-3 flex items-center shadow-xl border border-gray-800/50">
                <div className="relative flex-shrink-0">
                  <img
                    src={item.image_url || 'https://picsum.photos/seed/food/100/100'}
                ...
                <div className="flex-grow ml-4 min-w-0">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {tableNumber && (
                      <div
                        className={`flex items-center bg-gray-900 rounded-full p-1 border border-gray-800 ${
                          !isPlaceOpen || !item.is_available ? 'opacity-30 pointer-events-none' : ''
                        }`}
                      >
                        {cart[item.id] ? (
                          <div className="flex items-center">
                            <button
                              ...
                              className="p-1.5 text-accent hover:bg-gray-800 rounded-full transition-colors"
                            >
                              <Minus size={16} />
                            </button>
  ...
  if (!placeId)
    return (
      <div className="h-screen bg-background p-10 text-center flex flex-col items-center justify-center">
      ...
      <div className="h-screen bg-background p-6 flex flex-col items-center justify-center">
        <div className="text-center mb-8">
          ...
          <p className="text-text-secondary text-sm">
            Identificamos que você está na <strong>{labelSingular + ' ' + tableNumber}</strong>.
          </p>
        </div>
    ...
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      <header className="flex-shrink-0 p-4 bg-surface border-b border-gray-800 sticky top-0 z-50 flex justify-between items-center shadow-lg">
        <div className="flex items-center overflow-hidden">
          <button onClick={() => navigate(-1)} className="mr-3 p-2 hover:bg-gray-800 rounded-full text-text-secondary">
            <ChevronLeft size={24} />
          </button>
