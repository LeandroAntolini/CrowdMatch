...
const MenuPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (isAuthenticated && currentUser) {
          await fetchOrders();
          if (tableNumber) {
            await supabase.from('tables').upsert({ place_id: placeId, table_number: parseInt(tableNumber, 10), current_user_id: currentUser.id, last_activity: new Date().toISOString() }, { onConflict: 'place_id,table_number' });
          }
        }
      } catch (err) {
        console.error(err);
    ...
    loadData();
  }, [placeId, tableNumber, isAuthenticated, currentUser, fetchOrders]);
