...
import { ClipboardList, CheckCircle, Clock, Utensils, Phone, Check } from 'lucide-react';
...
const OrderBoardPage: React.FC = () => {
    ...
    const fetchOrders = async () => {
        if (ownedPlaceIds.length === 0) return;
                    ...
                    {orders.map(order => (
                        <div key={order.id} className={`bg-surface border-l-4 rounded-xl p-4 shadow-lg ${getStatusColor(order.status)}`}>
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <h2 className="text-xl font-bold">Mesa {order.table_number}</h2>
                                    <div className="flex items-center text-sm text-text-secondary">
                                        <span className="font-semibold text-text-primary mr-2">{order.profiles?.name}</span>
