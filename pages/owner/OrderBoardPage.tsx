  ...
  const tableMap = useMemo(() => {
    ...
    orders.forEach(order => {
      if (!map[order.table_number]) {
        // Se a mesa não existe no mapa (não foi configurada), cria uma entrada temporária
        map[order.table_number] = {
      ...
      if (!map[order.table_number].userName) {
        map[order.table_number].userName = order.profiles?.name || 'Cliente';
      }
    });
            ...
            return (
              ...
              >
                ...
                {m.pendingOrder ? (
                  ...
                  <User size={24} className={`${iconColor} mb-2`} />
                )}
