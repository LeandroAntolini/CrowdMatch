...
import { brazilianStates, citiesByState } from '../data/locations'; // Caminho atualizado
...
const ProfilePage: React.FC = () => {
    ...
    const handleToggleAvailability = async () => {
        ...
        const toastId = showLoading('Atualizando disponibilidade...');
        ...
        try {
            ...
            updateToast(toastId, 'success', 'Disponibilidade atualizada!');
        } catch (error: any) {
            updateToast(toastId, 'error', `Falha ao atualizar a disponibilidade: ${error.message}`);
        } finally {
    ...
    const handleSave = async () => {
        if (user) {
            ...
            try {
                ...
                updateToast(toastId, 'error', error.message || "Ocorreu um erro ao salvar o perfil.");
            } finally {
                setIsSaving(false);
            }
        }
    ...
    const handleDeletePhoto = async (indexToDelete: number) => {
        ...
        if (user.photos.length <= 1) {
            showError("Você precisa ter pelo menos uma foto.");
            return;
        }

        const photoPathToDelete = user.photos[indexToDelete]; // Agora armazena o path, não a URL completa
        ...
        setUser(updatedUser);
        const toastId = showLoading('Excluindo foto...');

        try {
            // Usar o photoPathToDelete diretamente
            const { error: removeError } = await supabase.storage
                .from('profile-photos')
                .remove([photoPathToDelete]);
            ...
            updateToast(toastId, 'success', 'Foto excluída com sucesso!');

        } catch (error: any) {
            console.error("Error deleting photo:", error);
            updateToast(toastId, 'error', `Não foi possível excluir a foto: ${error.message}`);
    ...
    const handleAddPhotoClick = () => {
        if (user && user.photos.length >= 5) {
            showError("Você pode enviar no máximo 5 fotos.");
            return;
        }
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const filePath = `${currentUser.id}/${Date.now()}.${fileExt}`; // Armazenar apenas o path
        ...
        if (uploadError) {
            ...
            updateToast(toastId, 'error', `Erro ao enviar a foto: ${uploadError.message}.`);
            setIsUploading(false);
            return;
        }

        // Obter a URL pública para exibição, mas armazenar o filePath
        const { data } = supabase.storage.from('profile-photos').getPublicUrl(filePath);
        setIsUploading(false);

        if (data?.publicUrl) {
            const newPhotos = [...user.photos, filePath]; // Armazenar o filePath
            const updatedUser = { ...user, photos: newPhotos };
            setUser(updatedUser);
            
            await updateUserProfile(updatedUser);
            updateToast(toastId, 'success', "Foto enviada e perfil atualizado com sucesso!");
        } else {
             updateToast(toastId, 'error', "Erro ao obter o URL público da foto.");
        }
        if (e.target) e.target.value = '';
    };

    // Função auxiliar para obter a URL pública a partir do filePath
    const getPublicPhotoUrl = (filePath: string) => {
        const { data } = supabase.storage.from('profile-photos').getPublicUrl(filePath);
        ...
        <div className="p-4 space-y-6">
            ...
            <div className="space-y-3">
                 <div className="relative w-full aspect-[4/5] bg-surface rounded-xl overflow-hidden shadow-lg">
                    <img src={getPublicPhotoUrl(user.photos[0])} alt="Foto principal do perfil" className="w-full h-full object-cover" />
                ...
                <div className="grid grid-cols-5 gap-2">
                     {user.photos.map((photoPath, index) => (
                        <div key={photoPath + index} className="relative aspect-square group">
                            <img src={getPublicPhotoUrl(photoPath)} alt={`Miniatura ${index + 1}`} onClick={() => handleSetMainPhoto(index)} className={`w-full h-full object-cover rounded-lg cursor-pointer transition-all duration-200 ${index === 0 ? 'ring-2 ring-accent ring-offset-2 ring-offset-background' : 'opacity-75 hover:opacity-100'}`} />
